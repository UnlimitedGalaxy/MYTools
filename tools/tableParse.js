const outputExcelData = function(file) {
...
* [
  ...
  需要保证有 导入XLSX
* */
function downloadExl(json, fileName = `马甲数据-${moment(new Date()).format('YYYYMMDDHHmmss')}.xlsx`) {
  ...
  let $a = $('<a></a>').attr('href', href).attr('download', fileName);
  $a[0].click();
